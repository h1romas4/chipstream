...
/// Stream a VGM file through VgmCallbackStream and log register writes/events.
///
...
/// `soundlog stream` to a callback stream built from `stream`, and drives it
/// to completion.
///
/// Shared by any command that wants register-stream logging regardless of how
/// its `VgmStream` was constructed — a parsed `.vgm` file (see
/// [`stream_vgm`]), or a lazily-generated stream (e.g. `soundlog mdx stream`).
