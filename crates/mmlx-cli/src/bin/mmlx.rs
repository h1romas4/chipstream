...
impl From<OutputFormat> for mmlx_cli::OutputFormat {
    fn from(format: OutputFormat) -> Self {
        match format {
            OutputFormat::Mdx => Self::Mdx,
            OutputFormat::Vgm => Self::Vgm,
        }
    }
}
...
enum MdxCommand {
    ...
    Compile {
        ...
        /// Output MDX or VGM binary file.
        output: PathBuf,

        /// Output format.
        #[arg(long, value_enum, default_value_t = OutputFormat::Mdx)]
    ...
    let result = match args.command {
        Command::Mdx { command } => match command {
            ...
            MdxCommand::Compile {
                ...
                output,
                output_format,
            } => mmlx_cli::compile_with_format(&input, &output, output_format.into()),
        },
        Command::Pdx {
