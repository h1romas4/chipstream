...
#[inline]
fn ym2608_write(builder: &mut VgmBuilder, instance: Instance, port: u8, register: u8, value: u8) {
...
/// Configure a YM2608 FM channel to produce a pure sine wave using only Operator 1.
///
/// Voice parameters:
...
pub fn write_ym2608_sine_voice(
    ...
    ym2608_write(builder, instance, port, 0x30 + ch, 0x01); // op1
    ym2608_write(builder, instance, port, 0x34 + ch, 0x01); // op2
    ym2608_write(builder, instance, port, 0x38 + ch, 0x01); // op3
    ym2608_write(builder, instance, port, 0x3C + ch, 0x01); // op4

    // TL  [---TL[6:0]]
    // OP1 at full volume (TL=0); OP2/3/4 fully attenuated (TL=127 = 0x7F).
    ym2608_write(builder, instance, port, 0x40 + ch, 0x00); // op1 – loudest
    ym2608_write(builder, instance, port, 0x44 + ch, 0x7F); // op2 – muted
    ym2608_write(builder, instance, port, 0x48 + ch, 0x7F); // op3 – muted
    ym2608_write(builder, instance, port, 0x4C + ch, 0x7F); // op4 – muted

    // RS/AR  [RS[1:0]-0-AR[4:0]]
    // RS=0, AR=31 (0x1F) → instant attack for all operators.
    ym2608_write(builder, instance, port, 0x50 + ch, 0x1F); // op1
    ym2608_write(builder, instance, port, 0x54 + ch, 0x1F); // op2
    ym2608_write(builder, instance, port, 0x58 + ch, 0x1F); // op3
    ym2608_write(builder, instance, port, 0x5C + ch, 0x1F); // op4

    // AM/D1R  [AM-0-D1R[4:0]]
    // AM=0, D1R=0 → no first-decay while key is held.
    ym2608_write(builder, instance, port, 0x60 + ch, 0x00); // op1
    ym2608_write(builder, instance, port, 0x64 + ch, 0x00); // op2
    ym2608_write(builder, instance, port, 0x68 + ch, 0x00); // op3
    ym2608_write(builder, instance, port, 0x6C + ch, 0x00); // op4

    // D2R  [---D2R[4:0]]
    // D2R=0 → no secondary decay.
    ym2608_write(builder, instance, port, 0x70 + ch, 0x00); // op1
    ym2608_write(builder, instance, port, 0x74 + ch, 0x00); // op2
    ym2608_write(builder, instance, port, 0x78 + ch, 0x00); // op3
    ym2608_write(builder, instance, port, 0x7C + ch, 0x00); // op4

    // D1L/RR  [D1L[3:0]-RR[3:0]]
    // D1L=0 (sustain at full level), RR=15 (fast release after key-off).
    // Encoded: (0 << 4) | 15 = 0x0F.
    ym2608_write(builder, instance, port, 0x80 + ch, 0x0F); // op1
    ym2608_write(builder, instance, port, 0x84 + ch, 0x0F); // op2
    ym2608_write(builder, instance, port, 0x88 + ch, 0x0F); // op3
    ym2608_write(builder, instance, port, 0x8C + ch, 0x0F); // op4

    // SSG-EG  [---SSG-EG[3:0]]
    // Proprietary; always 0.
    ym2608_write(builder, instance, port, 0x90 + ch, 0x00); // op1
    ym2608_write(builder, instance, port, 0x94 + ch, 0x00); // op2
    ym2608_write(builder, instance, port, 0x98 + ch, 0x00); // op3
    ym2608_write(builder, instance, port, 0x9C + ch, 0x00); // op4
    ...
    ym2608_write(builder, instance, port, 0xB0 + ch, 0x07);

    // B4H+  [L-R-AMS[1:0]-0-FMS[2:0]]
    // L=1, R=1 (both speakers), AMS=0, FMS=0.  Encoded: 0b11_00_0_000 = 0xC0.
    ym2608_write(builder, instance, port, 0xB4 + ch, 0xC0);
}

/// Write the 11-bit F-number and Block to a YM2608 FM channel.
///
/// Per the YM2608 manual, the high byte (A4H+) **must** be written before
/// the low byte (A0H+).
///
/// # Arguments
...
pub fn write_ym2608_frequency(
    ...
    ym2608_write(builder, instance, port, 0xA4 + ch, a4_value);
    ym2608_write(builder, instance, port, 0xA0 + ch, fnum_low);
}
...
pub fn write_ym2608_keyon(builder: &mut VgmBuilder, instance: Instance, port: u8, ch_in_port: u8) {
    assert!(ch_in_port < 3, "ch_in_port must be 0, 1, or 2");
    ...
    ym2608_write(builder, instance, 0, 0x28, 0xF0 | ch_bits);
}

/// Emit a key-off command that silences all four operators on one channel.
///
/// Same encoding as [`write_ym2608_keyon`] but with operator mask = 0.
pub fn write_ym2608_keyoff(builder: &mut VgmBuilder, instance: Instance, port: u8, ch_in_port: u8) {
    assert!(ch_in_port < 3, "ch_in_port must be 0, 1, or 2");
    let ch_bits = if port == 0 {
        ch_in_port
    } else {
    ...
    ym2608_write(builder, instance, 0, 0x28, ch_bits);
}

#[test]
fn test_ym2608_fm_keyon_and_tone_freq_matches_a4() {
    // Target pitch and chip configuration
    let target_hz = 440.0_f32;

    // YM2608 typical master clock
    let master_clock = 8_000_000.0_f32;

    // We'll use port 0, channel-in-port 0 → YM2608 channel 0 (the first FM channel).
    let port: u8 = 0;
    let ch_in_port: u8 = 0;

    // F-number calculation using OpnaSpec (prescaler = 1.0 for YM2608)
    ...
    // YM2608 F-number is 11 bits: low 8 → A0H, high 3 → A4H bits 2-0.
    ...
    builder.register_chip(Chip::Ym2608, Instance::Primary, master_clock as u32);

    // Initialize channel with a pure-sine voice
    write_ym2608_sine_voice(&mut builder, Instance::Primary, port, ch_in_port);

    // Write the frequency (A4H high byte first, then A0H low byte)
    write_ym2608_frequency(
        &mut builder,
        Instance::Primary,
        port,
        ch_in_port,
...
#[test]
fn test_ym2608_psg_channel_a_matches_a4() {
    // YM2608 typical master clock (use 8_000_000 as implemented)
    let master_clock = 8_000_000.0_f32;

    // PSG period formula: the YM2608 SSG section uses master_clock / 4 as its effective
    ...
    // Build VGM: register YM2608 (ported device), for PSG use port 0.
    let mut builder = VgmBuilder::new();
    builder.register_chip(Chip::Ym2608, Instance::Primary, master_clock as u32);

    // PSG registers are on port 0 (0x00/0x01 for channel 0)
    builder.add_chip_write(
        Instance::Primary,
        chip::Ym2608Spec {
            port: 0,
    ...
    // Mixer: on port 0, register 0x07
    builder.add_chip_write(
        Instance::Primary,
        chip::Ym2608Spec {
            port: 0,
            register: 0x07,
            value: 0b1111_1110u8, // enable channel A
        },
    );
    ...
    let doc = builder.finalize();
    let vgm_bytes: Vec<u8> = (&doc).into();
    super::maybe_write_vgm("ym2608_psg_a4.vgm", &vgm_bytes);

    // Callback stream + state tracking for YM2608
    let mut callback_stream = VgmCallbackStream::from_document(doc);
    callback_stream.track_state::<Ym2608State>(Instance::Primary, master_clock);
    ...
    assert!(
        got.is_some(),
        "Expected KeyOn StateEvent for YM2608 PSG but none captured"
    );
    ...
    assert!(
        ...
        "YM2608 PSG freq differs: got {} Hz, expected {} Hz (target {} Hz, diff {})",
        freq,
        expected_freq,
        TARGET_A4_HZ,
        diff
