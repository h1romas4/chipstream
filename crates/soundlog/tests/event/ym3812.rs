...
#[inline]
fn ym3812_write(builder: &mut VgmBuilder, instance: Instance, register: u8, value: u8) {
    builder.add_chip_write(instance, chip::Ym3812Spec { register, value });
}

/// Configure a YM3812 FM channel to produce a sine wave using additive synthesis.
///
/// OPL2 voice parameters for a basic sine wave:
...
pub fn write_ym3812_sine_voice(builder: &mut VgmBuilder, instance: Instance, channel: u8) {
    assert!(channel < 9, "YM3812 has 9 FM channels (0-8)");

    // OPL2 register layout:
    ...
    ym3812_write(builder, instance, 0x20 + op1, 0x21); // Modulator: EGT=1, MULT=1
    ym3812_write(builder, instance, 0x20 + op2, 0x2F); // Carrier: EGT=1, MULT=15 (muted via TL)

    // --- 0x40-0x55: KSL/TL (Key Scale Level / Total Level) ---
    // Bits: [KSL[1:0]-TL[5:0]]
    // TL: 0 = loudest, 63 = silent
    ym3812_write(builder, instance, 0x40 + op1, 0x00); // Modulator: full volume
    ym3812_write(builder, instance, 0x40 + op2, 0x3F); // Carrier: silent

    // --- 0x60-0x75: AR/DR (Attack Rate / Decay Rate) ---
    // Bits: [AR[3:0]-DR[3:0]]
    ym3812_write(builder, instance, 0x60 + op1, 0xFF); // Modulator: AR=15, DR=15 (fast)
    ym3812_write(builder, instance, 0x60 + op2, 0xFF); // Carrier: AR=15, DR=15

    // --- 0x80-0x95: SL/RR (Sustain Level / Release Rate) ---
    // Bits: [SL[3:0]-RR[3:0]]
    // SL=0 (sustain at full level), RR=15 (fast release)
    ym3812_write(builder, instance, 0x80 + op1, 0x0F); // Modulator: SL=0, RR=15
    ym3812_write(builder, instance, 0x80 + op2, 0x0F); // Carrier: SL=0, RR=15

    // --- 0xE0-0xF5: WS (Waveform Select) ---
    // 0=sine, 1=half-sine, 2=abs-sine, 3=quarter-sine
    ym3812_write(builder, instance, 0xE0 + op1, 0x00); // Modulator: sine wave
    ym3812_write(builder, instance, 0xE0 + op2, 0x00); // Carrier: sine wave

    // --- 0xC0-0xC8: Feedback/Algorithm ---
    // Bits: [--CNT-FB[2:0]-unused[2:0]]
    // CNT=1 (both operators to output = additive synthesis)
    // FB=0 (no feedback)
    ym3812_write(builder, instance, 0xC0 + channel, 0x01); // Additive, no feedback
}

/// Write the 10-bit F-number and 3-bit Block to a YM3812 FM channel.
///
/// # Arguments
/// * `channel`   – 0-8
/// * `fnum`      – 10-bit F-number (0-1023)
/// * `block`     – 3-bit block (0-7)
pub fn write_ym3812_frequency(
    ...
    ym3812_write(builder, instance, 0xA0 + channel, fnum_low);
    ...
    ym3812_write(builder, instance, 0xB0 + channel, block_fnum_high);
}

/// Emit a key-on command for a YM3812 FM channel.
///
...
/// Emit a key-off command for a YM3812 FM channel.
///
/// Same as keyon but with bit 5 cleared.
pub fn write_ym3812_keyoff(
    builder: &mut VgmBuilder,
    instance: Instance,
    channel: u8,
    fnum: u16,
...
#[test]
fn test_ym3812_fm_keyon_and_tone_freq_matches_a4() {
    // ---------------------------------------------------------------
    // Target pitch and chip configuration
    // ---------------------------------------------------------------
    let target_hz = 440.0_f32;

    // YM3812 typical master clock (NTSC)
    ...
    builder.register_chip(Chip::Ym3812, Instance::Primary, master_clock as u32);

    // 0. Enable waveform selection (register 0x01, bit 5)
    ym3812_write(&mut builder, Instance::Primary, 0x01, 0x20);

    // 1. Initialize channel with a sine wave voice
    write_ym3812_sine_voice(&mut builder, Instance::Primary, channel);

    // 2. Write the frequency (F-number and Block)
    write_ym3812_frequency(&mut builder, Instance::Primary, channel, fnum, block);

    // 3. Key on
    write_ym3812_keyon(&mut builder, Instance::Primary, channel, fnum, block);

    // 4. Hold note for ~0.5 s at 44 100 Hz sample rate
    builder.add_vgm_command(soundlog::vgm::command::WaitSamples(WAIT_SAMPLES));

    // 5. Key off
    write_ym3812_keyoff(&mut builder, Instance::Primary, channel, fnum, block);
