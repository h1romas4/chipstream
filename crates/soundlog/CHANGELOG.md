...
- **New**: Add `VgmStream`, an iterator-based, low-memory VGM stream processor that accepts raw VGM bytes (`push_data`) or a pre-parsed `VgmDocument` (`from_document`) and emits `VgmCommand` values incrementally. `VgmStream` handles DAC stream control, DataBlock storage/decompression, and automatically expands stream-generated chip writes during `Wait` periods.
