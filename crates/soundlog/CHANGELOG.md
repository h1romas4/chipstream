...
- **Breaking change (chip state API)**: `ChipState::read_register` no longer has a default implementation that returned `None`. Implementors must now provide an explicit `read_register` implementation. This avoids accidental always-None semantics and makes register-read behavior explicit for each chip state implementation. Update any custom `ChipState` implementations accordingly.
