...
- [x] Fix: `VgmExtraHeader` (`parser`, `to_bytes`) — corrected `chip_clock_offset` and `chip_vol_offset` to be relative to their own field positions within the extra header (per VGM spec), rather than relative to the start of the extra header. 
...
- [x] Change: Changed the parts where u64 was mistakenly used to usize.
- [x] Fix: Fixed an infinite loop occurring when loop_count reached saturation.

## v0.9.0

- [x] Change: `VgmHeader`: improve type-safety.
- [x] Add: `VgmHeaderMisc` struct. Expresses various other determinations made by VgmHeader.
- [x] Add: `VgmStream`: `from_vgm`, which uses raw VGM as storage.
- [x] Fix: `VgmStream`: respect VGM loop modifier.
- [x] Fix: `VgmStream`: respect DAC stream `LengthMode`(`Milliseconds`, `reverse`, `looped`).
...
- Fix: When `loop_offset` is set, the correct `loop_samples` is now configured during `finalize`.
