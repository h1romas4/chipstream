...
/// AY8910 stereo mask detailed structure.
///
/// The stereo mask is a bit mask with the following layout (bit 7 to bit 0):
/// `i y r3 l3 r2 l2 r1 l1`
///
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Ay8910StereoMaskDetail {
...
impl Ay8910StereoMaskDetail {
    /// Parse an AY8910 stereo mask byte into detailed structure.
    ///
    /// # Arguments
    /// * `mask` - The stereo mask byte (bit layout: i y r3 l3 r2 l2 r1 l1)
    ///
    /// # Examples
    ///
    /// ```
    /// use soundlog::vgm::detail::Ay8910StereoMaskDetail;
    ///
    /// // Enable all channels on both speakers for AY8910 instance 0
    ...
    /// assert!(mask.left_ch2 && mask.right_ch2);
    /// assert!(mask.left_ch3 && mask.right_ch3);
    /// ```
    ...
    /// Convert the detailed structure back to a stereo mask byte.
    ///
    /// # Examples
    ///
    /// ```
...
impl From<u8> for Ay8910StereoMaskDetail {
    fn from(mask: u8) -> Self {
        Self::from_mask(mask)
    }
}

impl From<Ay8910StereoMaskDetail> for u8 {
    fn from(detail: Ay8910StereoMaskDetail) -> Self {
        detail.to_mask()
    }
}

/// Parse an AY8910 stereo mask into detailed structure.
///
/// This function takes an `Ay8910StereoMask` and parses it into a detailed
/// structure with individual fields for each channel and speaker configuration.
///
/// # Arguments
/// * `mask` - The AY8910 stereo mask to parse
///
/// # Returns
/// * `Ay8910StereoMaskDetail` - Parsed stereo mask with detailed fields
///
/// # Examples
///
/// ```
/// use soundlog::vgm::command::Ay8910StereoMask;
/// use soundlog::vgm::detail::parse_ay8910_stereo_mask;
///
/// // YM2203 instance 1, channels 1 and 3 enabled on both speakers
...
/// assert!(detail.left_ch1 && detail.right_ch1);
/// assert!(!detail.left_ch2 && !detail.right_ch2);
/// assert!(detail.left_ch3 && detail.right_ch3);
/// ```
pub fn parse_ay8910_stereo_mask(
    mask: crate::vgm::command::Ay8910StereoMask,
) -> Ay8910StereoMaskDetail {
    Ay8910StereoMaskDetail::from_mask(mask.0)
}
